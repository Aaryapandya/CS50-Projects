...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for i in one_gene:
        if people[i]["mother"] == None:
            ...
            if mother in no_gene and father in no_gene:
                prob *= (1 - PROBS["mutation"]) * (1 - PROBS["mutation"]) + (1 - PROBS["mutation"]) * PROBS["mutation"]
            elif mother in no_gene and father in one_gene:
                prob *= (1 - PROBS["mutation"]) * .5 + PROBS["mutation"]*.5
            elif mother in no_gene and father in two_genes:
                prob *= (1 - PROBS["mutation"]) * (1-PROBS["mutation"]) + PROBS["mutation"] * PROBS["mutation"]

            elif mother in one_gene and father in no_gene:
                prob *= (1 - PROBS["mutation"]) * .5 + PROBS["mutation"]
            elif mother in one_gene and father in one_gene:
                prob *= .5*.5 + .5*.5
            elif mother in one_gene and father in two_genes:
                prob *= PROBS["mutation"] * .5 + .5*(1-PROBS["mutation"])
...
def normalize(probabilities):
    ...
    """
    for i in probabilities.keys():
        ngene = sum(probabilities[i]["gene"].values())
        ntrait = sum(probabilities[i]["trait"].values())

        for j in probabilities[i]["gene"].keys():
            probabilities[i]["gene"][j] /= ngene
        for j in probabilities[i]["trait"].keys():
            probabilities[i]["trait"][j] /= ntrait
